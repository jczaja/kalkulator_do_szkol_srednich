...
// TODO: can we get 88.5% from example e.g. half of percentage?
// TODO: zwolnienie z egzaminu
// TODO: zaimplementuje Cerificate
...
impl ExamResults<'_> {
    pub fn calculate_points(&self) -> Result<f32,&str>
    {
       ...
       Ok(self.polish.0 as f32 * 0.35 + self.math.0 as f32 * 0.35 + self.second_language.0 as f32 * 0.3) 
    }
}
...
impl ContestCuratorVoidship {
    pub const SubjectFinalist : ContestCuratorVoidship = ContestCuratorVoidship::AtLeastTwoTimesThematicLaureate; // Can it be two times the same subject?
    pub const ThematicFinalist : ContestCuratorVoidship= ContestCuratorVoidship::AtLeastTwoTimesThematicFinalist;                                                                               
}
...
impl CertificateResults<'_> {
    pub fn calculate_points(&self) -> Result<f32,&str>
    {
       if (2..=6).contains(&self.polish.0) || 
             ...
             (2..=18).contains(&self.achievements) { 
           return Err("Grade needs to be between 2 and 6");
       }
       ...
       Ok(get_course_points(self.polish.0) + get_course_points(self.math.0) +
...
mod tests {
    ...
    fn test_calculate_exam_points() -> Result<(),String> {
        ...
        assert_eq!(ExamResults{polish : (100, "Polish"), math : (100, "Math"), second_language : (100, "English")}.calculate_points(), Ok(100.0));
        assert_eq!(ExamResults{polish : (110, "Polish"), math : (100, "Math"), second_language : (100, "English")}.calculate_points(), Err("Score cannot be greater than 100"));
